...
### Командная смена: API для внешних ИИ-агентов студентов

Многопользовательский тренажёр (`/store/<id>/game/multi`) держит общую
серверную симуляцию: до 10 игроков, комнаты через `?room=<код>`.
Внешний ИИ (учебный агент студента) подключается по REST:

```bash
API=http://127.0.0.1:8050/api/mgame/st17/main

# 1. Войти в смену (kind: human | bot | api)
curl -X POST $API/join -H "Content-Type: application/json" \
  -d '{"name": "ИИ Иванова", "role": "cashier", "kind": "api"}'
# → {"player_id": "abc123...", "role": "cashier"}

# 2. Цикл: состояние смены → действие
curl "$API/state?player=abc123"
curl -X POST $API/action -H "Content-Type: application/json" \
  -d '{"player": "abc123", "action": "serve"}'
```

Действия: `move {x,y}` · `storeroom` (взять товар) · `restock
{target: индекс стеллажа}` · `serve` (встать за кассу) · `clean
{target: id разлива}` · `fix_fridge` · `paper` (замена ленты).
`GET state` возвращает полки с остатками, покупателей, очереди,
игроков, события (инкрементально через `?after=<id>`). Дополнительно:
`POST /bot {role}` — встроенный ИИ-бот, `POST /start` — старт смены.

Роли и лимиты (в сумме 10): `cashier`×2, `merch`×3, `cleaner`×2,
`tech`×2, `supervisor`×1. Скорость действия зависит от соответствия
роли (кассир обслуживает быстрее, мерчандайзер быстрее пополняет...).
Готовый пример агента: `python -m planogram3d.webapp.tools.demo_bot
--role merch --room main --start` — простая политика по роли, которую
студенты улучшают в заданиях. Итоговые баллы игроков с указанным ником
Roblox начисляются в командный лидерборд.

### Roblox: команда, бонусы и конвертация сцен

Игра-тренажёр интегрирована с Roblox в двух направлениях.

**1. Команда и поощрения.** Сотрудники регистрируются на брифинге
смены (имя + ник в Roblox); за пройденные смены и обучающие кейсы
начисляются баллы и бейджи («Кассир-новичок», «Хранитель чистоты»,
«Герой часа пик», «Наставник смены»), ведётся лидерборд и лента
поощрений. REST API:

```
POST /api/roblox/register  {name, roblox_user, store_id}
POST /api/roblox/progress  {name, roblox_user, store_id,
                            level, stars, revenue, stats}
GET  /api/roblox/team      → лидерборд + лента поощрений
```

Без настроек всё работает в режиме эмуляции (локальный реестр,
файл `PLANOGRAM_TEAM_FILE`, по умолчанию `planogram_team.json`).
Для реального Roblox-опыта задайте ключи Open Cloud:

```bash
export ROBLOX_API_KEY=<ключ Open Cloud>
export ROBLOX_UNIVERSE_ID=<id вселенной>
```

Тогда баллы сотрудников пишутся в стандартный DataStore `GourmanTeam`,
а каждое поощрение публикуется в MessagingService (топик
`gourman-rewards`) — сгенерированный place подписан на этот топик и
показывает поздравление игроку прямо в опыте, обновляя leaderstats.
Общение команды — штатный чат Roblox внутри опыта.

**2. Конвертор сцен → Roblox Studio.** Пакет `planogram3d.robloxkit`
генерирует place-файлы `.rbxlx`:

```bash
python -m planogram3d.robloxkit            # → robloxkit/demo/
python -m planogram3d.robloxkit --store st03 -o out/
```

* `gourman_store_<id>.rbxlx` — торговый зал: стеллажи с **реальной
  утверждённой планограммой** (блоки товаров в цветах поставщиков, с
  именами SKU), кассы, зона СКО, весы, холодильники, зоны скоропорта,
  склад, точка спауна — плюс Luau-скрипты тренажёра: задания через
  ProximityPrompt (склад → полки → касса), leaderstats с баллами,
  всплывающие объявления и подписка на топик поощрений
  `gourman-rewards`; ModuleScript `PlanogramData` содержит планограмму
  (SKU, цены, фейсинги) для собственных механик;
* `gourman_city.rbxlx` — обзорная сцена сети: магазины по реальным
  координатам города и логистический центр.

Файлы открываются в Roblox Studio (**File → Open from File**), после
Publish опыт доступен команде. Конвертор проверяет XML перед записью.

### Курьерское приложение: GPS-телеметрия и фискализация

Раздел «Интернет-заказы и доставка» (`/delivery`) принимает телеметрию
реального курьерского приложения:

```bash
curl -X POST http://127.0.0.1:8050/api/delivery/gps \
  -H "Content-Type: application/json" \
  -d '{"courier": "c1", "lat": 56.8600, "lon": 35.9000, "battery": 77}'
```

Пока координаты курьера приходят из приложения (интервал < 45 с),
эмуляция движения этого курьера отключается, а на карточке маршрута
источник помечается «приложение (реальный GPS)».

Два варианта курьерского приложения (поле `app` курьера):

| Вариант | Устройство | Фискализация чека при вручении |
|---|---|---|
| `android` | обычный Android-смартфон | чек пробивается **облачной кассой** магазина в момент вручения, покупателю уходит электронный чек (СМС/e-mail); печать — опционально на мобильном Bluetooth-принтере |
| `smartpos` | смарт-терминал **SmartOne** с фискальным накопителем (ФН) на борту | бумажный чек **печатается на месте вручения**, фискальные данные пишутся в ФН терминала, работает офлайн с последующей выгрузкой в ОФД |

Каждый чек содержит ФН/ФД/ФП и QR-строку формата 54-ФЗ; печатная
форма — `GET /receipt/<id>` (кнопка печати / `window.print()`).
Состояние конвейера (заказы, маршруты, план/факт для диаграммы Ганта,
чеки) — `GET /api/delivery/state`.

### Онлайн-табло прибытия: ИИ-прогноз ETA для пунктов доставки

Каждый пункт доставки видит на карте онлайн-табло времени прибытия
транспорта — по аналогии с «умными остановками» городского транспорта
(GPS-телеметрия машин → ИИ-модель → электронное табло, как в проекте
Urban Way в Бельцах):

* **магазин** — прибытие машин поставщиков и РЦ
  (клик по маркеру магазина → «🚏 Табло прибытия · ИИ»);
* **адрес покупателя** — прибытие курьера с позицией в очереди
  маршрута (клик по точке доставки на `/delivery`).

Прогноз считает онлайн-обучаемая модель `webapp/eta_ai.py`
(EWMA-оценки скорости и её дисперсии по классам транспорта,
обучаемые коэффициенты трафика по часу суток, длительность вручения);
модель дообучается на телеметрии каждого завершённого плеча — и на
эмулированной, и на реальной (`POST /api/delivery/gps`).

```bash
# табло магазина: машины поставщиков/РЦ в пути
curl http://127.0.0.1:8050/api/eta/store/st17
# → {"point": "«Гурман» №17 · Трёхсвятская", "rows": [
#     {"icon": "🚚", "label": "РЦ «Гурман»: Кефир 1% 0.9 л × 18",
#      "eta_ts": 1786909244.7, "eta_sec": 28, "sigma_sec": 7,
#      "plan_ts": 1786909233.9, "progress": 0.53,
#      "source": "ИИ-прогноз (модель обучена, 15 рейс.)"}, ...],
#    "model": {"speeds": {"truck": {"mps": 34.3, "obs": 15}}, ...}}

# табло адреса покупателя: курьер, очередь, ±σ
curl http://127.0.0.1:8050/api/eta/order/WEB-0011
# → {"point": "Фёдоров К. · ул. Вольного Новгорода, 57", "rows": [
#     {"icon": "🛵", "order": "WEB-0011", "this_point": true,
#      "queue_pos": 4, "eta_ts": ..., "eta_sec": 286, "sigma_sec": 21,
#      "plan_ts": ..., "source": "ИИ-прогноз · модель обучена (5 плеч.)",
#      "gps_source": "эмуляция"}, ...]}
```

Поля прогноза: `eta_ts`/`eta_sec` — ИИ-оценка прибытия,
`sigma_sec` — неопределённость ±1σ (копится по плечам маршрута),
`plan_ts` — плановое время для сравнения, `model` — сводка обученной
модели (скорости, дисперсии, коэффициенты часов, число наблюдений).
Те же прогнозы добавлены в `GET /api/delivery/state`: у каждой
остановки активного маршрута появляются `ai_arrive`, `ai_sigma`,
`ai_note`.
