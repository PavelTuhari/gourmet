...
Граф строится из того же ``static/data/chisinau_map.geojson`` (выгрузка
...
_REF_LAT = 47.02                               # широта Кишинёва — масштаб по умолчанию
_KX = 111320 * math.cos(math.radians(_REF_LAT))  # метров в градусе долготы (Кишинёв)
...
class RoadNet:
    def __init__(self, geojson_path: Path, ref_lat: float = _REF_LAT):
        ...
        # долготы = 111320*cos(широта)). Поэтому масштаб — атрибут
        # экземпляра: по умолчанию — широта Кишинёва (розничный контур),
        # а топливный контур с картой Молдовы передаёт свою ref_lat явно.
...
def cumulative(path: List[Node], kx: float = _KX) -> List[float]:
    ...
    ``kx`` по умолчанию — масштаб долготы Кишинёва (обратная совместимость
